...
(function() {
  ...
  buildReorderData = function(reorderItems) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      reorderRow = [snapshotDateFormatted || '', asin, productName, salesRank, productGroup, Number(totalUnitsShippedLast24Hours), Number(totalUnitsShippedLast7Days), Number(totalUnitsShippedLast30Days), Number(totalUnitsShippedLast90Days), Number(totalUnitsShippedLast180Days), Number(totalUnitsShippedLast365Days), numAfnNewSellers, removeFromRestockReport, Number(inStockOrOOSCrenstone), Number(inboundCrenstone), Number(daysOOSCrenstone), Number(last30DaysOfSalesWhenInStockCrenstone), Number(inStockOrOOSOredroc), Number(inboundOredroc), Number(daysOOSOredroc), Number(last30DaysOfSalesWhenInStockOredroc), Number(totalStockBothAccounts), Number(totalSalesBothAccounts30Days), seasonalTags, oemMfgPartNumber, oemMfg, vendorPartNumber, itemDescription, vendorName, parseFloat(vendorPrice), Number(quantityNeededPerASIN), parseFloat(totalPricePerASIN), Number(quantityNeededForRestockOrder3xOn30DaySales), Number(quantityNeededForRestockOrder6xOn30DaySales), closeoutRetailTag, canOrderAgain, sellingInAccounts, hasStockInAccounts, crenstoneSKU, crenstoneFNSKU, parseFloat(ourCurrentPriceInventoryCrenstone), parseFloat(lowestPrimePriceCrenstone), parseFloat(belowCurrentPriceCrenstone), brand, parseFloat(yourPrice), parseFloat(salesPrice), parseFloat(estimatedFeeTotal), parseFloat(estimatedFutureFee), parseFloat(estimatedShippingCost), parseFloat(totalInventoryCost), parseFloat(overheadRate), parseFloat(profit), parseFloat(futureProfit), Number(crenstoneUnitsShippedLast24Hours), Number(crenstoneUnitsShippedLast7Days), Number(crenstoneUnitsShippedLast30Days), Number(crenstoneUnitsShippedLast90Days), Number(crenstoneUnitsShippedLast180Days), Number(crenstoneUnitsShippedLast365Days), oredrocSKU, oredrocFNSKU, parseFloat(ourCurrentPriceInventoryOredroc), parseFloat(lowestPrimePriceOredroc), parseFloat(belowCurrentPriceOredroc), brand, parseFloat(yourPrice), parseFloat(salesPrice), parseFloat(estimatedFeeTotal), parseFloat(estimatedFutureFee), parseFloat(estimatedShippingCost), parseFloat(totalInventoryCost), parseFloat(overheadRate), parseFloat(profit), parseFloat(futureProfit), Number(oredrocUnitsShippedLast24Hours), Number(oredrocUnitsShippedLast7Days), Number(oredrocUnitsShippedLast30Days), Number(oredrocUnitsShippedLast90Days), Number(oredrocUnitsShippedLast180Days), Number(oredrocUnitsShippedLast365Days)];
  ...
  calculateCalculatedOutputs = function(data) {
    ...
    for (i = 0, len = data.length; i < len; i++) {
      ...
      row[21] = totalStock;
      row[22] = totalSales;
      row[31] = parseFloat(totalPriceOfASIN) !== 0 ? parseFloat(totalPriceOfASIN.toFixed(2)) : null;
      row[32] = quantityNeeded3x !== 0 ? quantityNeeded3x : null;
      row[33] = quantityNeeded6x !== 0 ? quantityNeeded6x : null;
      row[49] = parseFloat(totalPriceOfASIN) !== 0 ? parseFloat(totalPriceOfASIN.toFixed(2)) : null;
      row[70] = parseFloat(totalPriceOfASIN) !== 0 ? parseFloat(totalPriceOfASIN.toFixed(2)) : null;
      ...
      results.push(row[73] = "$" + parseFloat(futureProfit.toFixed(2)));
    }
    return results;
  };
