...
(function() {
  ...
  if (!config.IS_WORKER) {
    ...
    passport.deserializeUser(function(id, done) {
      return db.User.findByPk(id).then(function(user) {
  ...
  buildReorderData = function(reorderItems) {
    ...
    db.sequelize.query(selectQuery, {
      ...
      var asinRow, j, k, l, len1, len2, len3, manualInput, manualInputAsin, manualInputsByAsin, newReorderRows, ref1, ref2, row;
      newReorderRows = [];
      if (manualInputs.length > 0) {
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          ref2 = manualInputsByAsin[manualInputAsin];
          for (k = 0, len2 = ref2.length; k < len2; k++) {
            manualInput = ref2[k];
            row = _.findIndex(reorderData, function(data) {
              crenstoneSKU = manualInput['crenstone-sku'] || '';
              oredrocSKU = manualInput['oredroc-sku'] || '';
              return data[1] === manualInputAsin && data[38] === crenstoneSKU && data[59] === oredrocSKU;
            });
            if (row === -1) {
              asinRow = _.findIndex(reorderData, function(data) {
                return data[1] === manualInputAsin;
              });
              reorderRow = [reorderData[asinRow][0], manualInput['asin'], reorderData[asinRow][2], reorderData[asinRow][3], reorderData[asinRow][4], reorderData[asinRow][5], reorderData[asinRow][6], reorderData[asinRow][7], reorderData[asinRow][8], reorderData[asinRow][9], reorderData[asinRow][10], reorderData[asinRow][11], manualInput['remove-from-restock-report'], reorderData[asinRow][13], reorderData[asinRow][14], reorderData[asinRow][15], reorderData[asinRow][16], reorderData[asinRow][17], reorderData[asinRow][18], reorderData[asinRow][19], reorderData[asinRow][20], reorderData[asinRow][21], reorderData[asinRow][22], manualInput['seasonal-tags'], manualInput['oem-mfg-part-number'], manualInput['oem-mfg'], manualInput['vendor-part-number'], manualInput['item-description'], manualInput['vendor-name'], manualInput['vendor-price'], manualInput['quantity-needed-per-asin'], reorderData[asinRow][31], reorderData[asinRow][32], reorderData[asinRow][33], manualInput['closeout-retail-tag'], manualInput['can-order-again'], reorderData[asinRow][36], reorderData[asinRow][37], reorderData[asinRow][38], reorderData[asinRow][39], reorderData[asinRow][40], reorderData[asinRow][41], reorderData[asinRow][42], reorderData[asinRow][43], reorderData[asinRow][44], reorderData[asinRow][45], reorderData[asinRow][46], reorderData[asinRow][47], manualInput['estimated-shipping-cost'], reorderData[asinRow][49], manualInput['overhead-rate'], reorderData[asinRow][51], reorderData[asinRow][52], reorderData[asinRow][53], reorderData[asinRow][54], reorderData[asinRow][55], reorderData[asinRow][56], reorderData[asinRow][57], reorderData[asinRow][58], reorderData[asinRow][59], reorderData[asinRow][60], reorderData[asinRow][61], reorderData[asinRow][62], reorderData[asinRow][63], reorderData[asinRow][64], reorderData[asinRow][65], reorderData[asinRow][66], reorderData[asinRow][67], reorderData[asinRow][68], manualInput['estimated-shipping-cost'], reorderData[asinRow][70], manualInput['overhead-rate'], reorderData[asinRow][72], reorderData[asinRow][73], reorderData[asinRow][74], reorderData[asinRow][75], reorderData[asinRow][76], reorderData[asinRow][77], reorderData[asinRow][78], reorderData[asinRow][79]];
      ...
      for (l = 0, len3 = newReorderRows.length; l < len3; l++) {
        row = newReorderRows[l];
  ...
  calculateCalculatedOutputs = function(data) {
    var estimatedShippingCost, futureProfit, i, j, len, len1, overheadRate, profit, results, row, row2, totalPriceOfASIN, totalSales, totalStock;
    results = [];
